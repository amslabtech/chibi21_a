...
Astar::Astar():private_nh("~")
{
    ...
    pub_close_set = nh.advertise<geometry_msgs::PointStamped>("close_set_rviz",1);
}
...
{
    f = w * sqrt((x - goal_index.x)*(x - goal_index.x) + (y - goal_index.y)*(y - goal_index.y)); //wは重み
    return f;
}
