...
Local_Goal_Creator::Local_Goal_Creator():private_nh("~"){
    ...
    private_nh.param("reselection_dis",reselection_dis,{1.0});
    ...
    sub_current_pose = nh.subscribe("current_pose",10,&Local_Goal_Creator::current_pose_callback,this);
...
void Local_Goal_Creator::process(){
    ...
    while(ros::ok()){
        if(got_global_path){
            if(got_current_pose)set_the_next_goal();
            else std::cout<<"[ERROR]:LGC failed to get the pose"<<std::endl;
        }
        else std::cout<<"[ERROR]:LGC failed to get the path"<<std::endl;
